...
See [Terraform providers](https://learn.hashicorp.com/collections/terraform/providers) for the general information about creating custom providers from Terraform.
...
* repo-root
  ...
  * resource_usergroup.go: Defines the resource schema and methods for user groups.
...
Debugging the go-code when running from terraform is not supported. It is possible to print debug info as warnings in diag.Diagnostics. Debugging can also be done by writing a file with debug messages:
...
Github-actions is setup to aromatically build and publish new releases.
